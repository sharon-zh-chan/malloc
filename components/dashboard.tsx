      ...
      <header className="px-4 md:px-8 pt-6 pb-4 md:mr-24">
        ...
        <div className="mt-2 flex items-center gap-2">
          {editingTimeRange ? (
            ...
            >
              <span>
                {state.timeRange || "Set a time range..."}
              </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {state.blocks.map((block, index) => (
                <TodoBlockCard
                  ...
                  onUpdateTitle={(title) => updateBlockTitle(block.id, title)}
                  onAddItem={(text) => addItem(block.id, text)}
                  onToggleItem={(itemId) => toggleItemStatus(block.id, itemId)}
                  onDeleteItem={(itemId) => softDeleteItem(block.id, itemId)}
